...
function hyperedges_to_incidence_jl(hyperedges; num_nodes=nothing, one_based=true)
    ...
    n = isnothing(num_nodes) ? (max_node_in_data + offset) : max(num_nodes, max_node_in_data + offset)
    e = length(hyperedges)
...
function laplacian(H_jl, N_jl, M_jl, E_jl)
	degree(H_jl, N_jl, M_jl) - adjacency(H_jl, N_jl, M_jl, E_jl)
end
...
function t_eig_jl(A; order="LR", full_output=false)
    ...
    if full_output
        return V, S, Shift_Operator_Abs_Norm, Shift_Operator_Norm, V_fourier, S_fourier, flatten_D
    else
        return V, S
    end
end
